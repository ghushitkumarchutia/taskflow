...
import { processReport } from "../../jobs/report.job.js";
...
reportWorker.on("failed", async (job: Job | undefined, err: Error) => {
  if (!job) return;
  ...
  if (isTerminal) {
    ...
    console.error(`[REPORT] Job ${jobId} moved to dead letter queue`);
  } else {
    console.warn(`[REPORT] Job ${jobId} failed (attempt ${job.attemptsMade}/${maxAttempts}): ${err.message}`);
  }
});
